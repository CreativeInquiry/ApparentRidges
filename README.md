...
A list of samples generated by this algorithm for the standard test meshes (bunny, dragon, etc.) can be found in the [samples/](./samples) folder. They're downloaded from the [Stanford collection](http://graphics.stanford.edu/data/3Dscanrep/) and [Keenan Crane's collection](https://www.cs.cmu.edu/~kmcrane/Projects/ModelRepository/), and modified with [Meshlab](https://www.meshlab.net/) to a reasonable amount of triangles (50k-500k) in OBJ format. The modified version of the models used by this repo can be downloaded [here](https://glitch.com/edit/#!/standard-meshes).
